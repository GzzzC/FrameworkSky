...
public class Test3TableMgr : ITableData
{
    public Test3TableMgr(DynamicPacket packet)
    {
        Deserialize(packet);
    }

    private List<Test3Table> mList = new List<Test3Table>();
    
    public List<Test3Table> List
    {
        get {return mList;}
    }
    ...
    public void Deserialize (DynamicPacket packet)
    {
        ...
        for (int i = 0; i < num; i++)
        {
            Test3Table item = new Test3Table();
            item.Deserialize(packet);
            mList.Add(item);
        }
    }
    
    public Test3Table GetDataByIDAndName1(int id, string name1)
    {
        foreach (Test3Table data in mList)
        {
